        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td>
                <button class="btn btn-primary btn-sm" *ngIf="item.status === 'active' || item.status === 'pending'" (click)="acknowledge(item)">Acknowledge</button>
                <button class="btn btn-success btn-sm" *ngIf="item.status !== 'resolved' && item.status !== 'closed'" (click)="resolve(item)">Resolve</button>
              </td>
            </tr>
            <tr *ngIf="items.length === 0">
              <td colspan="5" class="empty">No alerts found</td>
            </tr>
          </tbody>
        </table>
...
export class AlertsComponent implements OnInit {
  ...
  acknowledge(item: Alert): void {
    this.api.put<any>(`/alerts/${item._id}/acknowledge`).subscribe({
      next: () => this.loadData()
    });
  }

  resolve(item: Alert): void {
    this.api.put<any>(`/alerts/${item._id}/resolve`).subscribe({
      next: () => this.loadData()
    });
  }
}
