...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        ...
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search patrol records..." class="search-input">
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            ...
            <tr *ngIf="items.length === 0">
              <td colspan="4" class="empty">No patrol records found</td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .badge-pending, .badge-scheduled { background: #fff3e0; color: #e65100; }
    .badge-missed, .badge-skipped { background: #ffebee; color: #c62828; }
    .badge-in_progress, .badge-ongoing { background: #e3f2fd; color: #1565c0; }
...
export class PatrolComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/patrol', params).subscribe({
      next: (res) => {
        this.items = res.data?.patrols || res.data || [];
