    ...
    <div class="page">
      <div class="page-header">
        ...
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search complaints..." class="search-input">
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              <td>{{ item.type }}</td>
              <td>{{ item.message | slice:0:60 }}{{ (item.message?.length || 0) > 60 ? '...' : '' }}</td>
...
export class ComplaintsComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/complaints', params).subscribe({
      next: (res) => {
        this.items = res.data?.complaints || res.data || [];
