  ...
  template: `
    <div class="page">
      <div class="page-header">
        <h2>Notes to Guard</h2>
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search notes..." class="search-input">
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              <td>{{ item.note | slice:0:80 }}{{ (item.note?.length || 0) > 80 ? '...' : '' }}</td>
              <td>{{ item.unit }}</td>
              <td>{{ item.createdBy }}</td>
              <td>{{ item.createdAt | date:'medium' }}</td>
            </tr>
            <tr *ngIf="items.length === 0">
              <td colspan="4" class="empty">No notes found</td>
            </tr>
          </tbody>
        </table>
...
export class NotesToGuardComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/notes-to-guard', params).subscribe({
      next: (res) => {
        this.items = res.data?.notes || res.data || [];
