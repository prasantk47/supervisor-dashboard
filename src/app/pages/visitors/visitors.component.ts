        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td>{{ item.checkOut ? (item.checkOut | date:'short') : '-' }}</td>
              <td>
            ...
            <tr *ngIf="items.length === 0">
              <td colspan="8" class="empty">No visitors found</td>
            </tr>
          </tbody>
        </table>
...
export class VisitorsComponent implements OnInit {
  ...
  approve(item: Visitor): void {
    this.api.put<any>(`/visitors/${item._id}/approve`).subscribe({
      next: () => this.loadData()
    });
  }

  reject(item: Visitor): void {
    this.api.put<any>(`/visitors/${item._id}/reject`).subscribe({
      next: () => this.loadData()
    });
  }
}
