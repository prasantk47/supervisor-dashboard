...
@Component({
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td><span class="badge" [ngClass]="'badge-priority-' + item.priority">{{ item.priority }}</span></td>
  ...
  styles: [`
    ...
    .badge-in_progress, .badge-acknowledged, .badge-assigned { background: #e3f2fd; color: #1565c0; }
...
export class HelpdeskComponent implements OnInit {
  ...
  assign(item: HelpdeskTicket): void {
    this.api.put<any>(`/helpdesk/${item._id}/assign`).subscribe({
      next: () => this.loadData()
    });
  }

  resolve(item: HelpdeskTicket): void {
    this.api.put<any>(`/helpdesk/${item._id}/resolve`).subscribe({
      next: () => this.loadData()
    });
  }
}
