...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        ...
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search incidents..." class="search-input">
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            ...
            <tr *ngIf="items.length === 0">
              <td colspan="4" class="empty">No incidents found</td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .badge-pending, .badge-open, .badge-reported { background: #fff3e0; color: #e65100; }
    .badge-rejected, .badge-cancelled, .badge-critical { background: #ffebee; color: #c62828; }
    .badge-in_progress, .badge-investigating { background: #e3f2fd; color: #1565c0; }
...
export class IncidentsComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/incidents', params).subscribe({
      next: (res) => {
        this.items = res.data?.incidents || res.data || [];
