...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        ...
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search billing..." class="search-input">
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td><span class="badge" [ngClass]="'badge-' + item.paymentStatus">{{ item.paymentStatus }}</span></td>
            </tr>
            <tr *ngIf="items.length === 0">
              <td colspan="4" class="empty">No billing records found</td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .badge-paid, .badge-completed { background: #e8f5e9; color: #2e7d32; }
    .badge-pending, .badge-unpaid { background: #fff3e0; color: #e65100; }
    .badge-overdue, .badge-failed { background: #ffebee; color: #c62828; }
...
export class BillingComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/billing', params).subscribe({
      next: (res) => {
        this.items = res.data?.bills || res.data || [];
