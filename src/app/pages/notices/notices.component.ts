...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        ...
        <div class="header-actions">
          <input type="text" [(ngModel)]="search" (ngModelChange)="onSearch()" placeholder="Search notices..." class="search-input">
          <button class="btn-create" (click)="createNew()">+ Create Notice</button>
        </div>
      </div>
        ...
        <table *ngIf="!loading">
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td>
                <button class="btn btn-primary btn-sm" (click)="view(item)">View</button>
              </td>
            </tr>
            <tr *ngIf="items.length === 0">
              <td colspan="4" class="empty">No notices found</td>
            </tr>
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .btn-create { padding: 8px 16px; background: #1a1a2e; color: #fff; border: none; border-radius: 6px; font-size: 14px; cursor: pointer; }
  `]
})
export class NoticesComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.api.get<any>('/notices', params).subscribe({
      next: (res) => {
        this.items = res.data?.notices || res.data || [];
  ...
  createNew(): void {
    // TODO: Open create notice dialog/modal
